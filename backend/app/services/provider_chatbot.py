...
def parse_vehicle_update(message: str, vehicles: list) -> dict:
    ...
    if is_route and not is_timing and not is_fare:
        ...
        # Check if they are updating both (route / from...to)
        # e.g., "from Jaipur to Delhi" or "route to Jaipur to Delhi" or "Jaipur to Delhi"
        ...
        # Single city update (only if we haven't already processed both)
...
def try_intercept_vehicle_update_flow(message: str, history: list[dict], provider_id: int, db: Session) -> str:
    ...
    if history:
        ...
        if assistant_msgs:
            ...
            if "do you want me to do it for you or do you want to do it manually" in last_assist.lower():
                asked_question = True
                # Find the user message right before that assistant message
                for i in range(len(history) - 1, -1, -1):
                    if history[i].get("role") == "assistant" and "do you want me to do it for you or do you want to do it manually" in history[i].get("content", "").lower():
                        # The user message before this assistant message is the original request
    ...
    if asked_question and original_update_message:
        ...
        if is_auto:
            ...
            # If they said something unrelated to the confirmation, fall back to parsing it as a new command
            pass
    ...
    # Check for question starters at the very beginning of the query
    ...
    # If it starts like a question and has no update verb, do not intercept
    ...
    # It is a parseable update command! Ask the question.
